 ...
 */
function validateToolInput(toolName: string, input: any, schema: any): { isValid: boolean; error?: string } {
  if (!input || typeof input !== 'object') {
    return { isValid: false, error: `Tool ${toolName} requires input parameters` };
  }

  // Check for specific tool requirements
  if (toolName === 'file_edit_diff') {
    const requiredParams = ['projectId', 'filename', 'diff'];
  ...
  if (schema?.properties) {
    ...
    const missingProps = requiredProps.filter((prop: string) => input[prop] === undefined);
 ...
 */
export function wrapToolWithValidation(toolName: string, tool: Tool<any, any>): Tool<any, any> {
  return {
    ...tool,
    execute: async (input: any, context?: any) => {
      ...
      if (!validation.isValid) {
        console.warn(`Tool validation failed for ${toolName}:`, validation.error);
 ...
 */
export function wrapToolsWithValidation(tools: Record<string, Tool<any, any>>): Record<string, Tool<any, any>> {
