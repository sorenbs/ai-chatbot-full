...
export function safeConvertToModelMessages(uiMessages: ChatMessage[]) {
  try {
    ...
    if (error instanceof Error && error.message.includes('incomplete tool input')) {
          ...
          const validParts = message.parts.filter(part => {
            if (part.type?.includes('tool-') && part.state === 'input-streaming') {
              ...
              if (!toolCall.input || typeof toolCall.input !== 'object') {
                console.warn(`Filtering malformed tool call: ${toolCall.toolCallId}`);
                return false;
              }
              
              // For file_edit_diff, we need at least filename and diff
              if (part.type === 'tool-file_edit_diff') {
                const hasRequiredParams = toolCall.input.filename && 
                  (toolCall.input.diff || toolCall.input.content);
                if (!hasRequiredParams) {
                  console.warn(`Filtering incomplete file_edit_diff: ${toolCall.toolCallId}`);
                  return false;
                }
              }
            }
